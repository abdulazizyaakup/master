...
class WebsiteSale(ProductConfiguratorController):
    ...
    @http.route('/tenderquote/get_keywords', type='http', auth="public", methods=['GET'], website=True, sitemap=False)
    def tag_read(self, q='', l=25, **post):
        data = request.env['ov.keyword'].search_read(
    ...
    def _get_mandatory_billing_fields(self):
        return ["name", "tb" , "keywords_ids" , "email", "street", "city", "country_id"]

    def _get_mandatory_shipping_fields(self):
        return ["name", "tb" , "keywords_ids" , "street", "city", "country_id"]
    ...
    def checkout_form_validate(self, mode, all_form_values, data):
        ...
        part = request.env['res.partner'].sudo().search([('email', '!=', False)])
        vals = []
        for p in part:
            vals.append(p.email)
        if (data.get('email') in vals):
            error["email"] = 'error'
            error_message.append(_('Email address exist! Please use another email address.'))
    ...
    def _checkout_form_save(self, mode, checkout, all_values):
        Partner = request.env['res.partner']
        if mode[0] == 'new':
            keywords = request.env['res.partner']
            keywords_ids = keywords._keyword_to_write_vals(checkout.get('keywords_ids', ''))
            checkout['keywords_ids'] = keywords_ids
            checkout['company_id'] = 2
            partner_id = Partner.sudo().create(checkout).id
            # print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX ", partner_id)
            # print("YYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY ", checkout['keywords_ids'])
            # print("ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ ", checkout)
        elif mode[0] == 'edit':
    ...
    def address(self, **kw):
        ...
        keyword = 'keyword_id' in values and values['keyword_id'] != '' and request.env['ov.keyword'].browse(int(values['keywords_ids']))
