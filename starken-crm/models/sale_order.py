...
SO_STATES = [
    ...
    ('sim_draft', 'SIM Draft'),
    ('sim_pending', 'SIM Pending Approval'),
...
class SaleOrder(models.Model):
    ...
    def _default_delivery_info(self):
        # if(self.company_id.id in [2,3,6,8]):
    ...
    ## signature ##
    signature_type = fields.Many2many('signature.type',string="Type")
    ## signature ##
    ...
    crm_project_id = fields.Many2one('crm.lead', string="Project")
    project_id = fields.Char(related='opportunity_id.crm_project_id', readonly=True, index=True)
    project_name = fields.Char(related='opportunity_id.name', readonly=True, index=True)
    ...
    # Fikri Add
    quot_remarks_starken = fields.Text(string='Remarks', default="Due to the continued and sharp increase in raw materials, we reserve the right to increase our rates proportionally by more than 10(%) according to market rates.")
    ...
    saleperson_id = fields.Many2one('res.partner', string="Saleperson")
    ...
    def _get_approval_status(self):
        for rec in self:
            if rec.state in ['draft', 'tba', 'cancel']:
                ...
                rec.approved_status = "Pending Approval"
    ...
    def get_revision_name(self):
        for rec in self:
            if rec.state in ['draft', 'tba', 'quotation', 'convert_so','sent', 'cancel']:
...
class SaleOrderLine(models.Model):
    ...
    plt_per_dlv = fields.Char(string='Pallets/delivery')
    ...
    price_policy = fields.Float('Price Policy', digits='Product Price',
                                 default=0.0)
    # Fikri add
    # start
    price_per_meter = fields.Float(string='Price/meter sq.')

    @api.onchange('product_id')
    def product_id_change_test(self):
        for rec in self:
            prod_name = rec.product_id.name
            if prod_name == False :
                pass
            elif len(prod_name) != 19 :              # epicor standard partnumber length of 19 
                rec.compressive_strength = ""
            else:
                sliced = prod_name[4:7]
                rec.compressive_strength = sliced+" MPa"
    ...
    def change_final_uni_price_on_up_changed(self):
        for line in self:
            ...
            if(line.price_unit < line.price_policy):
                warning = {
                        'title': _("Unit Price Below Pricing Policy Value!"),
                        'message': 'Are you sure to change unit price below pricing policy?'
                    }
                return { 'warning': warning}
                # _logger.warning('Are you sure to change unit price below pricing policy?')
    ...
    def product_id_change(self):
        for rec in self:
            rec.pro_group = rec.product_id.pro_group.id
            ...
            if self.product_id and self.product_uom_qty:
                ...
                if '-600x200x' in part_number:
                    ...
                    if price_policy:
                        ...
                        self.update({'price_unit': price_unit,'price_policy': price_unit})
    ...
    def _calculate_price_panel(self, toupdate=False):
        ...
        if price_to_use and product_id.net_vol:
            ...
            price_to_use = 0.0
        if toupdate:
            self.update({'price_unit': price_to_use,'price_policy': price_to_use})
            return False
        else:
            return price_to_use
