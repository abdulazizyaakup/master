...
class DrymixReport(models.TransientModel):

    _name = 'crm.drymix.report'
    _description = 'CRM Starken Drymix report'
    ...
    def drymix_report(self):
        view_mode = 'tree'
        tree_view_id = self.env.ref('project_crm_chg.crm_tree_view_midah_group').id

        return {
            'type': 'ir.actions.act_window',
            'name': _(' Result of Drymix report'),
            'view_type': 'form',
            'view_mode': 'tree,form',
            'res_model': 'crm.lead',
            'view_id': False,
            'views': [(self.env.ref('project_crm_chg.crm_tree_view_drymix_group').id, 'tree'),(self.env.ref('crm.crm_lead_view_form').id, 'form')],
            'context': {},
