...
class Lead2SubLeadBusinessUnit3(models.TransientModel):

    _name = 'crm.lead2sublead.businessunit2'


class Lead2SubLeadBusinessUnit2(models.TransientModel):

    _name = 'crm.lead2sublead.signature'
    ...
    team_id2 = fields.Many2one('crm.team', 'Team', index=True)
    company_id = fields.Many2one('res.company', string='Company', index=True, default=_default_company_id)
    ...
    bu_id = fields.Many2one('res.company', string="Company",index=True, tracking=True)
    crm_prj_team_leader = fields.Many2one('res.partner',string="Project Team Lead", domain="[('user_ids', '!=', False),('user_ids.company_id', '=', 14)]")
    # crm_prj_signature_sales = fields.Many2one('res.partner',string="Project Owner (Sales)", domain="[('user_ids', '!=', False),('user_ids.company_id', '=', 7)]")
    owner_marketing = fields.Many2one('res.partner',string="Project Owner (Marketing)", domain="[('user_ids', '!=', False),('user_ids.company_id', '=', 14)]")
    owner_sales = fields.Many2one('res.partner',string="Project Owner (Sales)", domain="[('user_ids', '!=', False),('user_ids.company_id', '=', 14)]")
    ...
    def createsublead_salesman2(self):
        ...
        self.env['crm.lead'].sudo().create({
                ...
                'crm_bci_id': bci_id,
                'name': name,
                'team_id2': self.team_id2.id,
                'crm_title': name,
                'company_id': self.company_id.id,
                # 'crm_prj_own_mark': self.owner_marketing.id,
                'crm_prj_team_leader': leader,
