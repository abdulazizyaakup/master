...
class Lead2SubLeadBusinessUnit1(models.TransientModel):

    _name = 'crm.lead2sublead.businessunit1'
    ...
    owner_marketing = fields.Many2one('res.partner',string="Project Owner (Marketing)", domain="[('user_ids', '!=', False),('user_ids.company_id', '=', 7)]")
    owner_sales = fields.Many2one('res.partner',string="Project Owner (Sales)", domain="[('user_ids', '!=', False),('user_ids.company_id', '=', 7)]")

    # @api.depends('crm_bci_id')

    def createsublead_salesman(self):
        ...
        self.env['crm.lead'].sudo().create({
                ...
                'crm_prj_own_sales': self.owner_sales.id,
