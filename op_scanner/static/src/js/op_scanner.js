odoo.define('op_scanner.op_scanner', function(require){
    ...
    var TwainContainer = AbstractAction.extend(ControlPanelMixin, {
        ...
        _SaveToFolder: function(){
            ...
            // console.log("This Machine Running : "+OSName);
            if(OSName == "Windows"){
                var locations = unescape("C:\\Users\\Public\\");
            }
            if(OSName == "Linux"){
                var locations = unescape("/opt/");
            }
            ...
            var save_doc = ajax.jsonRpc('/attachment/save/','call',{'title':title,'name': name,'description':description,'doc_name': doc_name, 'applicant_id':res_id, 'res_model':res_model,'fileType':fileType}).then(function(data){
                console.log("Success");
            });
            setTimeout(function(){ 
